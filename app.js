  ...
  try {
    ...
    const t = (key) => {
      if (!window.getTranslation) {
        console.error('❌ 翻譯系統未載入', {
          errorType: 'TranslationSystemError',
          timestamp: new Date().toISOString(),
          requestedKey: key,
          requestedLanguage: selectedLanguage,
          windowGetTranslation: typeof window.getTranslation,
          windowTranslations: typeof window.TRANSLATIONS,
          translationsJsLoaded: !!document.querySelector('script[src*="translations.js"]'),
          loadedScripts: Array.from(document.scripts).map(s => s.src),
          userAgent: navigator.userAgent
        });
        throw new Error(`翻譯系統載入失敗 - 請回報技術團隊。錯誤詳情: 翻譯鍵值=${key}, 語言=${selectedLanguage}, 時間戳=${new Date().toISOString()}`);
      }
      return window.getTranslation(selectedLanguage, key);
    };
